...
void MainWindow::on_pushButton_result_clicked() {
  if (ui->lineEdit->text().contains('x') && ui->lineEdit_x->text() != "") {
    ...
    int otvet = s21_SmartCalc(str, x, &result, mistake);
    if (otvet == 1) {
      QString m = mistake;
  ...
  if (!(ui->lineEdit->text().contains('x'))) {
    ...
    char mistake[100];
    double result;
    int otvet = s21_SmartCalc(str, 0, &result, mistake);
    if (otvet == 1) {
      QString m = mistake;
      ui->resultLabel->setText(m);
    } else {
  ...
  if (otvet == 1) {
    ...
    if (ui->Slowly->isChecked() == true) {
      ...
      for (X = Xmin; X <= Xmax; X = X + h) {
        s21_SmartCalc(str, X, &result, mistake);
        x.push_back(X);
        y.push_back(result);
      }
      ui->widget->addGraph();
...
void MainWindow::TimerSlot() {
  ...
  if (time_graphic <= 5 * N) {
    if (X <= Xmax) {
      otvet = s21_SmartCalc(str, X, &result, mistake);
      if (otvet == 1) {
        QString m = mistake;
        ui->resultLabel->setText(m);
      } else {
